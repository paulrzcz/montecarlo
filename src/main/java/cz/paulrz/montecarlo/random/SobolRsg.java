...
public class SobolRsg {
    ...
    public SobolRsg(final int dimensionality, final long seed, final DirectionIntegers direction) {
        ...
        if (this.dimensionality > maxTabulated) {
            final MersenneTwister uniformRng = new MersenneTwister(seed);
            for (int k = maxTabulated; k < this.dimensionality; k++) {
                for (int l = 1; l <= degree[k]; l++) {

                    do {
                        // u is in (0,1)
                        final double u = uniformRng.nextDouble();
    ...
    private void skipTo(final /*@NonNegative*/ long skip) {
        final long n = skip + 1;
        final long ops = (long) (Math.log(n) / M_LN2) + 1;
    ...
    public final double[] nextSequence() /* @ReadOnly */ {
    ...
    public final double[] lastSequence() /* @Read-only*/ {
        return sequence;
    }
