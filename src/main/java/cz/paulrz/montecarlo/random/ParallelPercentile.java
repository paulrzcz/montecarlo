 ...
 * <p>
 * Since 2.2, ParallelPercentile uses only selection instead of complete sorting
...
public class ParallelPercentile extends AbstractUnivariateStatistic implements Serializable {
    ...
    /**
     * Constructs a ParallelPercentile with a default quantile
     * value of 50.0.
     */
    public ParallelPercentile() {
        this(50.0);
    }

    /**
     * Constructs a ParallelPercentile with the specific quantile value.
     * @param p the quantile
     * @throws IllegalArgumentException  if p is not greater than 0 and less
     * than or equal to 100
     */
    public ParallelPercentile(final double p) {
        setQuantile(p);
        cachedPivots = null;
    }
     ...
     */
    public ParallelPercentile(ParallelPercentile original) {
        copy(original, this);
    }
    ...
    private class SelectWorker implements Callable<Double> {

        private final double[] work;
        private final int[] pivotsHeap;
        private final int k;

        public SelectWorker(final double[] work, final int[] pivotsHeap, final int k) {
            this.work = work;
            this.pivotsHeap = pivotsHeap;
            this.k = k;
        }

        public Double call() throws Exception {
            return select(work, pivotsHeap, k);
        }
    }
     ...
     */
    private static double select(final double[] work, final int[] pivotsHeap, final int k) {
     ...
     */
    static int medianOf3(final double[] work, final int begin, final int end) {
     ...
     */
    @Override
    public ParallelPercentile copy() {
        ParallelPercentile result = new ParallelPercentile();
        copy(this, result);
        return result;
    }
     ...
     *
     * @param source ParallelPercentile to copy
     * @param dest ParallelPercentile to copy to
     * @throws NullArgumentException if either source or dest is null
     */
    public static void copy(ParallelPercentile source, ParallelPercentile dest)
